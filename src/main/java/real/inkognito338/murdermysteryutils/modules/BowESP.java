...
public class BowESP extends Module {
    ...
    public BowESP() {
        ...
        this.addSetting(new Setting("Mode", SettingType.MODE, "Stand", modes));

        this.addSetting(new Setting("Distance", SettingType.NUMBER, 50.0, 5.0, 150.0));
    ...
    private boolean hasBow(EntityArmorStand stand) {
        ...
        return main.getItem() == Items.BOW || off.getItem() == Items.BOW;
    }
    ...
    private void drawBowBox(Entity e, float r, float g, float b, float a, double size, boolean outline, boolean isDropped) {
        ...
        if (isDropped) {
            ...
            if (main.getItem() == Items.BOW) {
                centerX = x + 0.3; // сдвиг вправо от центра стойки (примерно)
            } else if (off.getItem() == Items.BOW) {
